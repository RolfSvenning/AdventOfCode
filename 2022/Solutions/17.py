...
def simulate(rocksCount, partTwo=False):
    ...
    H = {} # heights at time i
    jetsCount = len(jets)
    states = {} # state -> buildings placed

    i = 0
    j = -1
    while True:
        j += 1
        jet = jets[j % jetsCount]
        s = shapes[i % 5]
        n, m = np.shape(s)
        ...
        if ((B[sX - n : sX, sY: sY + m] + s) > 1).any():
            ...
            state = tuple([np.where(B[:,index] == 1)[0][0] for index in range(1, 1 + 7)] + [i % 5, j % jetsCount])
            if state in states and not foundCycle:
                last_i = states[state]
                cycleLen = (i - last_i)
