...
S = frozenset([(x - 1, y - 1, input[x][y]) for x,y in [(x + 1, y + 1) for x in range(n) for y in range(m)] if input[x][y] in "<>^v"])
...
def updateState(S):
    ...
    return frozenset([updateBlizzard(b) for b in S])
...
def BFS(start_time, end):
    ...
    for p, ps in Q:
        ...
        for q, qs in adjMoves((p, ps)):
            ...
            if len(Q) % 10000 == 0: print("len(Q):", len(Q))
